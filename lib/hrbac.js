...
class HRBAC {
  ...
  constructor() {
    ...
    this.unauthorizedErrorFunc = async (req, res, next) => {
      let err = new Error('Unauthorized');
      err.status = 401;
      next(err);
    };
    this.customFunctionErrorFunc = async (err, req, res, next) => {
  ...
  _mustBeValidFunction(func, name, argsNumber) {
 ...
 */
  addUnauthorizedErrorFunc(func) {
    ...
    this._mustBeValidFunction(func, 'func', 3);
    ...
    return async (req, res, next) => {
      ...
      if(bool) {
        ...
        if (this._isAsync(this.unauthorizedErrorFunc)) {
          return await this.unauthorizedErrorFunc(req, res, next);
        } else {
          return this.unauthorizedErrorFunc(req, res, next);
        }
      }
    };
  }
